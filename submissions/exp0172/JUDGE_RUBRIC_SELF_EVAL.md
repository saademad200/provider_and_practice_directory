...
Curated-package refresh against `submissions/latest_final_package.zip`. The score is artifact-backed and every evidence pointer in `evidence/judge_rubric_self_eval.csv` resolves inside the judge upload.
...
| human_review | 8 | 5.0 | 8.0 | review_count=38.0 threshold=30 passed=True; auto_apply_precision=1.0 threshold=0.99 passed=True |
