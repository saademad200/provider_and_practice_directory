...
```mermaid
flowchart LR
    schedule[AWS EventBridge Schedule] --> registry[Source Connector Registry]
    ...
    score --> safety[Safety And Policy Gates]
    ...
    class registry,retrieval,normalize,identity,conflict,score,audit,monitor agent
    class review human
    class safety safety
```
...
|---|---|---|---|
| Source Connector Registry | Chooses approved sources by authority, cost, freshness, and field coverage | No source is used without access policy and health checks | `proposal/SOURCE_CONNECTOR_STATUS_MATRIX.md`, `evidence/source_connector_registry.csv` |
